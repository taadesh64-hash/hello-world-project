...
export default function ReaderPage() {
  ...
  const fetchAllChapters = useCallback(async () => {
    try {
      ...
      if (!response.ok) {
        console.error(`❌ [Reader] Failed to fetch chapters. Status: ${response.status}`);
        throw new Error(`HTTP ${response.status}: Failed to fetch chapters`);
      }
      
      const data = await response.json();
      console.log(`✅ [Reader] Successfully fetched ${data.data?.length || 0} chapters`);
      return data.data || [];
    } catch (err: any) {
  ...
  const fetchChapterPages = useCallback(async (chapterId: string) => {
    try {
      ...
      if (!response.ok) {
        console.error(`❌ [Reader] Failed to fetch pages. Status: ${response.status}`);
        throw new Error(`HTTP ${response.status}: Failed to fetch pages`);
      }
      
      const data = await response.json();
      console.log(`✅ [Reader] Successfully fetched ${data.data?.pages?.length || 0} pages for chapter ${chapterId}`);
